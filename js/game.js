...
function createMonsterFace(monsterColor) {
  ...
  // Add teeth for scarier appearance
...
function createDevilHorns(monsterColor) {
  ...
  // Add pulsing animation to horn tips
  const animate = () => {
    const time = Date.now() * 0.001;
...
function createBigAntenna(monsterColor) {
  ...
  // Add pulsing animation to antenna tip
  const animate = () => {
    const time = Date.now() * 0.001;
...
function createExit() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    // Randomize particle positions around the door
  ...
  doorFrame.userData.animate = () => {
    ...
    haloParticles.children.forEach((particle, index) => {
      // Create a floating and swirling motion
  ...
  if (!validPosition) {
    ...
    doorFrame.position.set(0, 1, 0);  // Fallback position
  }

  // Add exit door to scene
  scene.add(doorFrame);
  exit = doorFrame;

  // Update UI to show exit is available
  updateObjectivesUI();
}
...
function animate() {
  ...
  if (!gameOver && !won && !isPaused && controls.isLocked) {
    ...
    // Animate exit door if it exists
